...
fn create_quinn_endpoint(role: Role) -> quinn::Endpoint {
    // TODO: Don't panic
    let certificate = rustls::Certificate(include_bytes!("cer.pem").to_vec());
    let private_key = rustls::PrivateKey(include_bytes!("key.pem").to_vec());

    let mut endpoint_config = quinn::EndpointConfig::default();
    endpoint_config.grease_quic_bit(true);
    let mut transport_config = quinn::TransportConfig::default();
    transport_config
        .max_concurrent_uni_streams(quinn::VarInt::from_u32(1000))
        .max_idle_timeout(None);
    let endpoint_config = std::sync::Arc::new(endpoint_config);

    let server = match role {
        Role::Active => false,
        Role::Passive => true,
    };
    let server_config = if server {
        let mut server_config =
            quinn::ServerConfig::with_single_cert(vec![certificate], private_key).unwrap();
        server_config
            .transport_config(std::sync::Arc::new(transport_config))
            .use_retry(false)
            .migration(true);
    ...
    let allow_mtud = false;
    quinn::Endpoint::new(endpoint_config, server_config, allow_mtud)
}
